...
For continuous or ordinal covariates, the "mean" column provides the mean
and the "var" column the standard deviation of the covariate.
For categorical covariates, the "mean" column provides the number
and the "var" column the percentage of samples with the given covariate "value".

If a value for the variable of interest is given in the "variable" column
(only for categorical variables), then the statistics apply
only to the corresponding samples.
...
```{r ewas-selected-sites-option}
child.filename <- file.path(path, "missing.rmd")
if (nrow(ewas.summary$selected.cpg.stats$p.value) > 0)
    child.filename <- file.path(path, "ewas-report-selected-cpg-sites.rmd")
```

```{r ewas-selected-sites, child=child.filename}
```
