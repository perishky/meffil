...
```{r controlmeans-graph, fig.height=24}
...
if (!is.null(qc.summary$cell.counts))
    child.filename <- file.path(path, "cell-counts.rmd")
```
...
suppressMessages(qc.summary$genotype.summary$graphs$snp.beta)
...
if (!is.null(qc.summary$genotype.summary$graphs$snp.concordance))
    child.filename <- file.path(path, "genotype-concordance.rmd")
```

```{r genotype-concordance, child=child.filename}
```
