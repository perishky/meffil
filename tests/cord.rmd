...
## Compare Andrews and Bekulski cord blood counts to minfi

```{r cord-minfi, cache=T}
#source("http://bioconductor.org/biocLite.R")
#biocLite("FlowSorted.CordBlood.450k")
if (require(FlowSorted.CordBlood.450k)) {
    library(minfi)
    raw.minfi <- read.450k.exp(base = path)
    source("estimate-cell-counts.r")
    counts.minfi <- estimateCellCounts.new(raw.minfi,
...
```{r}
if (require(FlowSorted.CordBlood.450k)) {
    counts.meffil <- counts[["andrews and bakulski cord blood"]]
    rownames(counts.minfi) <- sub("_.*", "", rownames(counts.minfi))
    counts.minfi <- counts.minfi[rownames(counts.meffil), colnames(counts.meffil)]
    print(sapply(rownames(counts.meffil), function(i) cor(counts.meffil[i,], counts.minfi[i,])))
    print(sapply(colnames(counts.meffil), function(i) cor(counts.meffil[,i], counts.minfi[,i])))
    print(cbind(colMeans(counts.meffil), colMeans(counts.minfi)))
}
```
...
knitr:::kable(comparison.table(results, "isva"), row.names=F)
```

Similarly for SVA EWAS:
```{r, results="asis"}
knitr:::kable(comparison.table(results, "sva"), row.names=F)
```
...
First for ISVA EWAS:
```{r, results="asis"}
compare.p.value.ranks <- function(results) {
    ranks <- sapply(names(results), function(variable.name, method) {
...
knitr:::kable(compare.p.value.ranks(results, "isva"), row.names=F)
```

Second for SVA EWAS:
```{r, results="asis"}
knitr:::kable(compare.p.value.ranks(results, "sva"), row.names=F)
```
