...
```{r, results="hide"}
counts.meffil.objects <- mclapply(norm.objects,
                                  meffil.estimate.cell.counts,
                                  reference=meffil.gse35069.references()$standard,
                                  verbose=T)
counts.meffil <- t(sapply(counts.meffil.objects, function(x) x$counts))
```
...
```{r, dev="CairoPNG"}
meffil.cell.count.qc.plot(counts.meffil.objects, meffil.gse35069.references()$standard)
```
