...
7 of the 71 sites are missing.

## Duplicated probes

Several thousand probes appear in duplicate across the microarray.
For example, cg06373096 appears `r length(grep("cg06373096",rownames(beta.meffil)))` times.
Here is the correlations between these duplicates:

```{r}
cor(t(beta.meffil[grep("cg06373096",rownames(beta.meffil)),]))
colMeans(cor(t(beta.meffil[grep("cg06373096",rownames(beta.meffil)),])))
```
