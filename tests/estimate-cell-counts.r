## code for estimating cell counts
## from the most recent version of minfi.
## included here because that version of minfi
## requires the absolute most latest versions of R and bioconductor.

require(genefilter)
require(matrixStats)

estimateCellCounts.new <- function (rgSet, compositeCellType = "Blood", processMethod = "auto",
                                probeSelect = "auto",
    ...
    counts <- projectCellType.new(getBeta(mSet)[rownames(coefs), ], coefs)
...
pickCompProbes <- function(mSet, cellTypes = NULL, numProbes = 50, compositeCellType = compositeCellType, probeSelect = probeSelect) {
    ...
    if(ncol(phenoDF) == 2) { # two group solution
        ...
        tmp <- validationCellType.new(Y = p, pheno = phenoDF, modelFix = form)
...
projectCellType.new <- function(Y, coefCellType, contrastCellType=NULL, nonnegative=TRUE, lessThanOne=FALSE){
...
validationCellType.new <- function(Y, pheno, modelFix, modelBatch=NULL,
