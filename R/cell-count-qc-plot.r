...
#' 
#' @export 
meffil.cell.count.qc.plot <- function(count.objects, reference) {
    if (is.null(names(count.objects))) names(count.objects) <- paste0("s", 1:length(count.objects))

    beta <- cbind(reference$beta, sapply(count.objects, function(object) object$beta))
    ...
    dat$col <- dat$sample %in% colnames(reference$beta)
    ...
    list(betas=beta.plot, counts=count.plot)
}
